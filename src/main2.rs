...
static AUDIO_BUFFERS: Mutex<EspRawMutex, RefCell<AudioBuffers<16384, 16384>>> =
...
fn as_u16_slice(slice: &[u8]) -> &[u16] {
    unsafe { core::slice::from_raw_parts(slice.as_ptr() as *const _, slice.len() / 2) }
}

fn as_u8_slice_mut(slice: &mut [u16]) -> &mut [u8] {
    unsafe { core::slice::from_raw_parts_mut(slice.as_mut_ptr() as *mut _, slice.len() * 2) }
}

fn as_u16_slice_mut(slice: &mut [u8]) -> &mut [u16] {
    unsafe { core::slice::from_raw_parts_mut(slice.as_mut_ptr() as *mut _, slice.len() / 2) }
}
